pub use crate::{
    ...
    impls::types::{Courier, CourierId, Data, FoodOrderError, Restaurant, RestaurantId},
...
use super::types::{CourierResult, ManagerResult, RestaurantResult};
    ...
    T: Storage<Data> + Storage<ownable::Data>,
{
    ...
    default fn add_restaurant(
        ...
        phone_number: String,
            ...
            FoodOrderError::RestaurantAlreadyExist
        );
        ensure!(restaurant_name.len() > 0, FoodOrderError::InvalidNameLength);
        ensure!(
            restaurant_address.len() > 0,
        ...
        self.data::<Data>()
            .restaurants
            .insert(&restaurant_id, &restaurant);
        self.data::<Data>()
            .restaurant_account_id
            .insert(&restaurant_account, &restaurant_id);
    ...
    default fn add_courier(
        ...
        phone_number: String,
            ...
            !self
                .data::<Data>()
                .courier_account_id
                .contains(&courier_account),
            FoodOrderError::CourierAlreadyExist
        );
        ensure!(courier_name.len() > 0, FoodOrderError::InvalidNameLength);
        ensure!(
            courier_address.len() > 0,
            FoodOrderError::InvalidAddressLength
        );
        ensure!(
            phone_number.len() > 0,
        ...
        self.data::<Data>()
            .courier_account_id
            .insert(&courier_account, &courier_id);

        // Emit `AddCourierEvent`
        self.emit_add_courier_event(courier_id, courier_name, courier_address, phone_number);
    ...
    #[modifiers(only_owner)]
    default fn change_manager(&mut self, new_account: AccountId) -> ManagerResult {
    ...
    #[modifiers(only_owner)]
    default fn change_fee_rate(&mut self, rate: u32) -> ManagerResult {
