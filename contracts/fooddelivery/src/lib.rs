...
mod inksmartcontract {
    ...
    use logics::{
        impls::{
            customer_service::CustomerServiceEvents,
            courier_service::CourierServiceEvents,
            restaurant_service::RestaurantServiceEvents,
            manager_service::ManagerServiceEvents,
            types::*, 
            types::Data,
        },
        traits::{
    ...
    impl CustomerService for FoodDelivery {}
    impl CourierService for FoodDelivery {}
    impl RestaurantService for FoodDelivery {}
    impl ManagerService for FoodDelivery {}
    impl GetService for FoodDelivery {}
    ...
    impl CustomerServiceEvents for FoodDelivery {
    ...
    impl CourierServiceEvents for FoodDelivery {
    ...
    impl RestaurantServiceEvents for FoodDelivery {
    ...
    impl ManagerServiceEvents for FoodDelivery {
    ...
    mod e2e_tests {
        ...
        async fn new_works(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            // given
            let constructor = FoodDeliveryRef::new();
            ...
            // when
            let get_owner = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
        ...
        async fn change_manager_works(mut client: ink_e2e::Client<C,E>) -> E2EResult<()> {
            ...
            let change_manager = build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| {
                fooddelivery.change_manager(
                    account,
                )
            });
            ...
            // get the owner
            let get_owner = 
                build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| fooddelivery.get_owner());
        ...
        async fn change_fee_rate_works(mut client: ink_e2e::Client<C,E>) -> E2EResult<()> {
            ...
            // change fee rate
            let change_fee_rate = build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| {
                fooddelivery.change_fee_rate(
                    rate,
                )
            });
            ...
            // get the fee rate
            let get_fee_rate = build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| fooddelivery.get_fee_rate());
        ...
        async fn main_works(mut client: ink_e2e::Client<C,E>) -> E2EResult<()> {
            ...
            // Add restaurant
            let add_restaurant = build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| {
                fooddelivery.add_restaurant(
                    restaurant_account,
            ...
            let get_restaurant_all = 
                build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| fooddelivery.get_restaurant_all(1, 2));
            ...
            // Add courier
            let add_courier = build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| {
                fooddelivery.add_courier(
                    courier_account,
                    courier_name.clone(),
            ...
            // get the courier
            let get_courier_all = 
                build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| fooddelivery.get_courier_all(1, 2));
            ...
            // Add Food
            let add_food = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {fooddelivery.add_food(
                    food_name.clone(),
                    description.clone(),
                    price,
                    eta,
            ...
            // get the food
            let get_food = 
                build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| fooddelivery.get_food_all(1,2));
            ...
            // Add customer
            let add_customer = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {
                    fooddelivery.add_customer(
                        customer_name.clone(),
            ...
            //get the customer
            let get_customer = 
                build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| fooddelivery.get_customer_all(1,2));
            ...
            // submit order
            let submit_order = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {
                    fooddelivery.submit_order(
                        food_id,
            ...
            // get the order
            let get_order = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| fooddelivery.get_order_all(1,2));
            ...
            // update Food
            let update_food = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {fooddelivery.update_food(
                    food_id,
                    food_name.clone(),
                    description.clone(),
                    price,
            ...
            // confirm order
            let confirm_order = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {fooddelivery.confirm_order(
                    order_id,
                    eta,
                )
            });
            ...
            // get the delivery
            let get_delivery = 
                build_message::<FoodDeliveryRef>(contract_acc_id.clone()).call(|fooddelivery| fooddelivery.get_delivery_all(1,2));
            ...
            let finish_cook = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {fooddelivery.finish_cook(
                    order_id,
                    )
                });
            ...
            let pickup_delivery = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {
                    fooddelivery.pickup_delivery(
                        delivery_id,
                    )
                });
            ...
            let delivery = get_delivery_result.return_value().unwrap()[0].clone();
            assert_eq!(DeliveryStatus::PickUp, delivery.status);
            let order = get_order_result.return_value().unwrap()[0].clone();
            ...
            let deliver_order = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {fooddelivery.deliver_order(
                    order_id,
                )
            });
            ...
            let accept_delivery = build_message::<FoodDeliveryRef>(contract_acc_id.clone())
                .call(|fooddelivery| {fooddelivery.accept_delivery(
                    delivery_id,
                )
            });
            ...
            let delivery = get_delivery_result.return_value().unwrap()[0].clone();
            assert_eq!(DeliveryStatus::Accept, delivery.status);
